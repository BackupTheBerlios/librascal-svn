...
bool getaddr::dump(char *&dst, unsigned int &size)
{
	...
	debug((flog, rl_resolver, "%X: built a getaddr dns packet %u (%s): qr=%u, opcode=%u, aa=%u, tc=%u, rd=%u, ra=%u, z=%u, rcode=%u, qdcount=%u, ancount=%u, nscount=%u, arcount=%u", get_id(), hdr.id, data, hdr.qr, hdr.opcode, hdr.aa, hdr.tc, hdr.rd, hdr.ra, hdr.z, hdr.rcode, hdr.qdcount, hdr.ancount, hdr.nscount, hdr.arcount));
	return true;
}
