...
rrid_t rascal_init(unsigned int policy)
{
	...
	if (!rascal_isok(rascal_set_option(RO_THREAD_POLICY, policy))) {
		debug((flog, rl_misc, "could not initialize the library (policy: %u)", policy));
		return REC_SUCCESS;
	}
