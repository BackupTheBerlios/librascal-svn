...
static int usage(void)
{
	fprintf(stderr, "%s",
		...
		" -p proto   : protocol name (forces SRV RR resolution, requires -s)\n"
		" -q         : quiet (for using within scripts)\n"
		" -r count   : set the number of retry attempts\n"
		" -s service : service name (forces SRV RR resolution, requires -p)\n"
...
int main(int argc, char * const * argv)
{
	...
	sock_t sock;
	bool single = false;
	const char *host = NULL, *proto = NULL, *svc = NULL;
	...
	if ((proto != NULL && svc == NULL) || (proto == NULL && svc != NULL))
		return usage();
